...
mdf_prep <- mdf_prep %>% 
  ...
  mutate(growth_cap = case_when(
    growth == "before" ~ "Before",
    growth == "after" ~ "After",
    growth == "regrowth" ~ "Regrowth")) %>% 
  mutate(growth_cap = factor(growth_cap, levels = c("Before", "After", "Regrowth")))
...
taxa_plot <- new_groups$mdf %>% 
  ...
  theme_bw() +
  facet_grid(~growth_cap, scales = "free") +
...
taxa_plot_un <- new_groups$mdf %>% 
  filter(room_type == "untreated") %>% 
  mutate(room_type_cap = "Untreated") %>% 
  plot_microshades(., pseudo_order$cdf, group_label = "Phylum-Genus", x = "new_id") + 
  theme_bw() +
...
# Focusing on outlier sink
```{r}

mdf_prep <- prep_mdf(ps_decon, subgroup_level = "genus")

mdf_prep_p6 <- mdf_prep %>% 
  filter(before_after != "NA") %>% 
  filter(room == "P6") %>% 
  mutate(day_f = factor(day, levels = c("1", "8", "15B", "16", "17", "18", "25", "29", "36")))


# plotting
color_df <- create_color_dfs(mdf_prep_p6, group_level = "phylum", subgroup_level = "genus", selected_groups = c("Pseudomonadota", "Actinomycetota", "Bacteroidota", "Bacillota"), cvd = TRUE)
...
p6_plot <- new_groups$mdf %>% 
  filter(!(is.na(before_after))) %>% 
  filter(room_type != "untreated") %>%
  mutate(day_f = factor(day, levels = c("1", "8", "15B", "16", "17", "18", "25", "29", "36"))) %>% 
  plot_microshades(., pseudo_order$cdf, group_label = "Phylum-Genus", x = "day_f") + 
  theme_bw() +
  #facet_grid(~growth, scales = "free") +
  theme(panel.background = element_blank(), axis.line = element_line(colour = "black"), strip.background = element_rect(fill="white"), legend.position = "none") + 
  ylab("Relative Abundance") + 
  xlab("Day")

plot_grid(p6_plot, cust_legend, nrow = 2, rel_heights = c(2,1))
...
# plotting
color_df <- create_color_dfs(mdf_prep_r, group_level = "phylum", subgroup_level = "genus", selected_groups = c("Pseudomonadota", "Actinomycetota", "Bacteroidota", "Bacillota"), cvd = TRUE)
...
regrowth_plot <- new_groups$mdf %>% 
  filter(!(is.na(before_after))) %>% 
  filter(room_type != "untreated") %>%
  mutate(day_f = factor(day, levels = c("1", "8", "15B", "16", "17", "18", "25", "29", "36"))) %>%
  plot_microshades(., pseudo_order$cdf, group_label = "Phylum-Genus", x = "day_f") + 
  theme_bw() +
  facet_wrap(~room, nrow = 12, ncol = 2) +
  theme(panel.background = element_blank(), axis.line = element_line(colour = "black"), strip.background = element_rect(fill="white"), legend.position = "none") + 
  ylab("Relative Abundance") + 
  xlab("Day")

plot_grid(regrowth_plot, cust_legend, nrow = 2, rel_heights = c(4,1))
...
alpha_pairwise <- alpha_long %>%
  ...
  filter(!(is.na(growth))) %>% 
  group_by(metric) %>%
  pairwise_wilcox_test(value ~ growth, paired = TRUE, p.adjust.method = "fdr")
...
rm(list = setdiff(ls(), c("ps_decon_g","ps_nomc_g", "adiv", "new_figure3")))

ps_noc <- ps_decon_g %>% 
  subset_samples(., room != "negswab" & sample_type != "mc")
  
otu_mat <- as.matrix(otu_table(ps_noc))
comm_df <- as.data.frame(t(otu_mat))

rc_res <- RC.pc(comm_df, rand = 99, na.zero = TRUE, nworker = 8,
...
rc_long <- rc_mat %>%
  ...
  pivot_longer(-Sample1, names_to = "Sample2", values_to = "RC") %>%
  filter(Sample1 != Sample2) %>% 
  rename(new_id = Sample1)


meta <- data.frame(sample_data(ps_noc))


rc_test <- rc_long %>%
  left_join(meta, by = "new_id") %>%
  left_join(meta, by = c("Sample2" = "new_id"), suffix = c(".1", ".2")) %>%
  mutate(
    same_sink = room.1 == room.2,
    same_phase = growth.1 == growth.2,
    comparison = case_when(
      same_sink & same_phase ~ "same sink within phase",
      same_sink & !same_phase ~ "same sink across phases",
      !same_sink & same_phase ~ "different sinks within phase",
      TRUE ~ "different sinks across phases"))


rc_test %>% 
  #filter(same_sink == TRUE) %>% 
  filter(sample_type.1 == "untreated" | growth.1 == "before") %>% 
  ggplot(., aes(x = comparison, y = RC, colour = sample_type.1)) +
  geom_jitter() +
  ylab("Raup–Crick (RC_bray)") +
  theme_minimal() +
...
ggplot(rc_test, aes(x = RC, fill = comparison)) +
  geom_density(alpha = 0.4) +
  labs(x = "RC", y = "Density") +
  geom_vline(xintercept = 0.95, color = "red") +
  theme_minimal() + facet_wrap(~comparison, ncol = 1, scales = "free") + 
  xlim(-1,1) + theme(legend.position = "none")


rc_test %>% 
  filter(sample_type.1 != "untreated" & sample_type.2 != "untreated") %>%
  ggplot(., aes(y = RC, x = comparison, color = comparison)) +
  geom_violin() +
  geom_jitter(alpha = 0.4) +
  labs(x = "", y = "RC") +
  theme_minimal() + 
  ylim(-1,1) + theme(legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1))
...
rc_summary <- rc_test %>%
  group_by(comparison) %>%
  summarise(median_RC = median(RC), prop_deterministic = mean(RC > 0.95), n_pairs = n())
...
adonis2(dist_bray ~ growth + room_type, data = meta_treated, permutations = 999, by = "terms")
...
pseud_plot <- plot_df %>% 
  ...
  geom_boxplot() +
  geom_jitter() +
  facet_wrap(~room_type, scales = "free_x", strip.position = "top") +
  theme_minimal() +
  theme(plot.title = element_text(face = "italic", hjust = 0.5), legend.position = "none", axis.text.x = element_text(color = c("#66997B", "#c1af8e", "#CC727B"))) +
  scale_colour_manual(values = c("before" = "#66997B", "after" = "#c1af8e", "regrowth" = "#CC727B")) +
...
stat.test <- plot_df %>%
  ...
  group_by(room_type) %>% 
  pairwise_wilcox_test(
    abundance ~ growth, p.adjust.method = "fdr", paired = TRUE)

p_plot <- pseud_plot + stat_pvalue_manual(stat.test, label = "p.adj", y.position = c(0.6, 0.72, 0.82))
