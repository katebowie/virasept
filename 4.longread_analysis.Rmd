...
df <- df %>% 
  mutate(ARG_coverage = ARG_length*MAG_coverage)
...
save.image(file = here("Data", "longread.RData"))
...
perc_bp <- df %>%
  select(barcode, MAG_Identifier, HiFi_Yield_bp, Genome_Size, MAG_coverage) %>% 
  unique() %>% 
  group_by(barcode, HiFi_Yield_bp) %>% 
  summarise(total_MAG_bp = sum(MAG_coverage*Genome_Size)) %>% 
  mutate(perc = (total_MAG_bp/HiFi_Yield_bp)*100) %>% 
  select(barcode, perc)
...
length(unique(df$Best_Hit_ARO)) # 96 ARGs (NA makes 97)
...
arg_abund_plot <- arg_abund %>%
  ...
  group_by(Best_Hit_ARO) %>% 
  summarise(plot_count = regrowth - before, plot_perc = ((regrowth-before)/before)*100) %>% 
...
arg_perc_plot <- arg_abund_plot %>% 
  mutate(perc_change = plot_perc) %>% 
  filter(plot_count != 0) %>% 
  ggplot(., aes(x = Best_Hit_ARO, y = perc_change, fill = plot_count > 0)) +
  scale_fill_manual(values = c("TRUE" = "#DEC3C6", "FALSE" = "#CBDCD2"),
                    labels = c("Before", "Regrowth")) +
  geom_col() +
  coord_flip() +
  labs(y = "Percent Change of ARG Counts", x = "Efflux Pump ARGs", fill = "phase") +
  theme_bw() + theme(legend.position = "none")
...
### stats for differential plots

```{r}

arg_abund_sample <- df_resist %>%
  filter(mech_type == "Efflux") %>% 
  distinct(MAG_ID, new_contig, Start, Stop, room, growth, Best_Hit_ARO, Species, barcode) %>%
  count(growth, Best_Hit_ARO, barcode, name = "total_count") %>% 
  pivot_wider(., names_from = Best_Hit_ARO, values_from = total_count, values_fill = 0)

wilcox.test(arg_abund_sample$`Pseudomonas aeruginosa soxR`[arg_abund_sample$growth == "before"], arg_abund_sample$`Pseudomonas aeruginosa soxR`[arg_abund_sample$growth == "regrowth"], paired = TRUE)

wilcox.test(arg_abund_sample$`Acinetobacter baumannii AbaQ`[arg_abund_sample$growth == "before"], arg_abund_sample$`Acinetobacter baumannii AbaQ`[arg_abund_sample$growth == "regrowth"], paired = TRUE)

arg_abund_sample %>%
  select(growth, adeF, barcode) %>% 
  group_by(growth) %>%
  summarise(mean = mean(adeF), 
            median = median(adeF),
            sd = sd(adeF), total = sum(adeF))
...
rm(list = setdiff(ls(), c("plasmid_count", "prop_carb", "Fig4_top", "ht_grob", "arg_dif_plot", "arg_perc_plot", "rpkm_sr_plot", "Fig4b", "mag_taxa_s")))
...
efflux_boxplots <- plot_grid(arg_perc_plot, plasmid, labels = c("D", ""), nrow = 1, rel_widths = c(2, 1.2))

Fig4 <- plot_grid(Fig4_half, efflux_boxplots, nrow = 2, rel_heights = c(2,1))

ggsave("my_plot_large.eps",
       plot = Fig4,
       width = 9,      # in inches
       height = 10,      # in inches
