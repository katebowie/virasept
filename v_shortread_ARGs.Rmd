...
arg_read_plot <- sr_arg + stat_pvalue_manual(stat.test, label = "p.adj", y.position = c(450000, 500000, 550000)) + ylim(NA, 580000)
...
all_samples <- amr_treated %>%
  distinct(new_id, growth, room)

adeF <- amr_treated %>%
  filter(Best_Hit_ARO == "adeF") %>%
  group_by(new_id, growth, room) %>%
  summarise(sum_RPKM = sum(RPKM))

adeF <- all_samples %>%
  left_join(adeF, by = c("new_id", "growth")) %>%
  mutate(sum_RPKM = replace_na(sum_RPKM, 0))

kruskal.test(sum_RPKM ~ growth, data = adeF)

stat.test <- pairwise.wilcox.test(adeF$sum_RPKM, adeF$growth, paired = TRUE, p.adjust.method = "fdr")


stat.test <- adeF %>%
  ungroup() %>% 
  mutate(growth = droplevels(growth)) %>%
  pairwise_wilcox_test(sum_RPKM ~ growth, p.adjust.method = "fdr")

a_plot <- ggplot(adeF, aes(x = growth, y = sum_RPKM)) + geom_boxplot() +
  theme_bw() + ylab("RPKM") + xlab("") + ggtitle("adeF")


adef <- a_plot + stat_pvalue_manual(stat.test, label = "p.adj", y.position = c(1500, 1640, 1800))
...
all_samples <- amr_treated %>%
  distinct(new_id, growth)

pseudo <- amr_treated %>%
  filter(Best_Hit_ARO == "Pseudomonas aeruginosa soxR") %>%
  group_by(new_id, growth) %>%
  summarise(sum_RPKM = sum(RPKM))

pseudo <- all_samples %>%
  left_join(pseudo, by = c("new_id", "growth")) %>%
  mutate(sum_RPKM = replace_na(sum_RPKM, 0))

kruskal.test(sum_RPKM ~ growth, data = pseudo)

pseudo$growth <- factor(pseudo$growth, levels = c("before", "after", "regrowth"))
...
p_plot <- ggplot(pseudo, aes(x = growth, y = sum_RPKM)) + geom_boxplot() +
  theme_bw() + ylab("RPKM") + xlab("") + ggtitle("P.aeruginosa soxR")


soxr <- p_plot + stat_pvalue_manual(stat.test, label = "p.adj", y.position = c(300, 350, 400))
...
rpkm_efflux <- ef_pumps + stat_pvalue_manual(stat.test, label = "p.adj.signif", y.position = c(8000, 9500, 11000)) + ggtitle("Efflux Pumps")
...
plot_grid(rpkm_efflux, soxr, labels = c("A", "B"), nrow = 1)
...
arg_diversity_df <- arg_diversity_df %>% 
  left_join(amr_treated %>% select(new_id, growth), by = "new_id") %>% 
  unique()


Shannon_plot <- arg_div %>% 
  filter(growth != "NA") %>% 
  filter(!(is.na(growth))) %>% 
...
kruskal.test(Shannon ~ growth, data = arg_diversity_df)

stat.test <- arg_diversity_df %>% 
  filter(growth != "NA") %>% 
  filter(!(is.na(growth))) %>% 
  mutate(growth = droplevels(growth)) %>% 
  pairwise_wilcox_test(Shannon ~ growth,
    p.adjust.method = "fdr")

Shannon <- Shannon_plot + stat_pvalue_manual(stat.test, label = "p.adj", y.position = c(4.5, 5, 5.5)) + ylim(NA, 6)
...
kruskal.test(Simpson ~ growth, data = arg_diversity_df)
...
stat.test <- arg_diversity_df %>% 
  ...
  filter(!(is.na(growth))) %>% 
  mutate(growth = droplevels(growth)) %>%
  pairwise_wilcox_test(Simpson ~ growth,
    p.adjust.method = "fdr")

Simpson <- Simpson_plot + stat_pvalue_manual(stat.test, label = "p.adj", y.position = c(52, 57, 63)) + ylim(NA, 70)
