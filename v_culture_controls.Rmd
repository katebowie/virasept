...
meta %>%
  ...
  filter(room_type != "untreated") %>%
  group_by(room, c_day, plate_type, growth) %>%
  summarise(total_CFU = sum(CFU_swab)) %>% 
  ungroup() %>% 
  group_by(plate_type, growth) %>% 
  summarise(med_CFU = median(total_CFU), med_st = sd(total_CFU), min_cfu = min(total_CFU), max_cfu = max(total_CFU))

# log-change reduction after disinfection
meta %>%
  filter(!is.na(plate_type)) %>%
  filter(new_day %in% c("15V", "16")) %>% 
  filter(room_type != "untreated") %>%
  group_by(room, c_day, plate_type, growth) %>%
  summarise(total_CFU = sum(CFU_swab)) %>% 
  ungroup() %>% 
  pivot_wider(., id_cols = c(plate_type, room), values_from = total_CFU, names_from = c_day) %>% 
  group_by(plate_type, room) %>% 
  reframe(log_reduc = abs(log10(`15D` / `16`))) %>% 
  summarise(max_log = max(log_reduc), min_log = min(log_reduc))


meta %>% 
  filter(!is.na(plate_type)) %>%
  filter(room_type != "untreated" & growth == "after") %>%
  filter(count_plate < 1) %>% 
  group_by(plate_type) %>% 
  summarise(count = n())
...
untreated_fig <- untreated_df %>% 
  ...
  stat_compare_means(aes(group = sink), 
                     method = "wilcox.test",
                     label = "p.format", label.y = 8.3) + xlab("") +
...
meta %>% 
  group_by(growth) %>% 
  reframe(median_ss = median(SampleSums), avg_ss = mean(SampleSums), range_ss = range(SampleSums))
...
read_sum <- growth_plot + stat_pvalue_manual(stat.test, label = "p.adj", y.position = c(4.8e7, 5.5e7, 6.3e7))
...
rm(list = setdiff(ls(), c("meta", "ps_clean", "ps_decon", "read_sum")))
